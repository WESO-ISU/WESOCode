...
//TESTING SESSION 2 FULL CODE 
//UPDATE TO USE WRITEMICROSECONDS 1000-2000 FOR MORE PRECISION 
//CONSIDER ALWAYS LEAVING 1280 ON!
#include <Servo.h>
#include <SD.h>
...
int RECORD_BTN_PIN = 27;
int LA_GO_HOME_BTN_PIN = 26;

const int LA_EXTEND = 1550; //This should be the furthest out the linear actuator can move. Adjust as needed. 
const int LA_RETRACT = 1050; //This should be the furthest in the linear actuator can move. Adjust as needed.
//All linear actuator movement needs to be bounded by these numbers. This is to avoid strain on linear actuator.
const int LA_STEP_SIZE = 5; //Common step size for moving linear actuator. Look into changing this to decimal values to give us more places to move to.
const int LA_HOME_POSITION = 1500; //This is the home position of the linear actuator. Subject to change.
int la_current_position = 1500; //This also acts as the starting position for la. Remeber to change this value whenever the linear actuator is moved to ensure position stays in bounds.

// --- Linear Actuator Microsecond PWM Settings ---
const int LA_US_MIN = 1050;   // Fully retracted pulse width
const int LA_US_MAX = 1550;   // Fully extended pulse width

// Convert logical LA position → PWM microseconds
int laPosToMicros(int pos) {
    return map(pos, LA_RETRACT, LA_EXTEND, LA_US_MIN, LA_US_MAX);
}
const int E_STOP_POSITION = 1100; // Change this value to the correct e-stop value, 83 is likely incorrect. 
...
const LookupEntry lookupTable[LOOKUP_TABLE_SIZE] = {
    ...
    { 12.0f, 110, 160.0f, 0b00011111 } //This shi should be a bit longer. we NEED to get this jawn looking nice during testing monday 4/13/2026
};
...
void handle_actuator_buttons(){
    ...
    if(extendPressed && la_current_position + LA_STEP_SIZE < LA_EXTEND){
        ...
        // --- Microsecond-accurate actuator movement ---
...
void handle_la_go_home(){
  ...
  if(laGoHome){
    //linearActuator.write(LA_HOME_POSITION); 
    // --- Microsecond-accurate home position ---
    linearActuator.writeMicroseconds(LA_HOME_POSITION);
...
*/ 
// --- Write actuator position using microsecond-accurate PWM ---
void handle_actuator_write(int write_value){
    if (is_legal_la_write(write_value)) {
...
void loop(){
    ...
    Serial.print(" ");
    Serial.print("LA POS: ");Serial.println(la_current_position);
    */
    int la_us = laPosToMicros(la_current_position);

    Serial.print("RPM: ");
    Serial.print(get_rpm());
    Serial.print("   ");

    // --- Print the real PWM microsecond value instead of logical position ---
    Serial.print("LA PWM(us): ");
    Serial.println(la_us);
