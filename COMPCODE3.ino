...
// These must always be (min=LA_DUTY_EXTEND, max=LA_DUTY_RETRACT) regardless of naming.
const int LA_DUTY_MIN = LA_DUTY_EXTEND;   // 3835 - lower bound
...
const LookupEntry lookupTable[LOOKUP_TABLE_SIZE] = {
    ...
    {  5.0f,       3830,    1280.0f,         0x80,      200.0f }, //#FLAG tune spike delta
    {  6.0f,       3836,    88.0f,           0xF0,      200.0f }, //#FLAG tune spike delta
    {  7.0f,       3839,    15.1f,           0xF8,      200.0f }, //#FLAG tune spike delta
    {  8.0f,       3842,    15.1f,           0xF8,      200.0f }, //#FLAG tune spike delta
    {  9.0f,       3852,    13.29f,          0xD6,      200.0f }, //#FLAG tune spike delta
    { 10.0f,       3857,    11.04f,          0x7F,      200.0f }, //#FLAG tune spike delta
    { 11.0f,       3857,    11.04f,          0x7F,      200.0f }, // mirrors 10 m/s //#FLAG tune spike delta
    { 12.0f,       3861,    11.04f,          0x7F,      200.0f }, // mirrors 10 m/s //#FLAG tune spike delta
    { 13.0f,       3861,    11.04f,          0x7F,      200.0f }, // mirrors 10 m/s //#FLAG tune spike delta
};
...
const float RPM_MARGIN                   = 25.0f;    //#FLAG tune stability window after testing
...
const float RPM_WIND_INCREMENT_THRESHOLD = 30.0f;    //#FLAG tune - RPM rise that signals 1 m/s wind increase
...
void la_write(int duty) {
    duty = constrain(duty, LA_DUTY_MIN, LA_DUTY_MAX);
...
bool is_legal_la_duty(int duty) {
    return (duty >= LA_DUTY_MIN && duty <= LA_DUTY_MAX);
}
...
void handle_actuator_buttons() {
    ...
    // Extend = decrease duty, retract = increase duty (higher value = more retracted)
    if (extendPressed && la_current_duty - LA_DUTY_STEP >= LA_DUTY_MIN) {
        la_write(la_current_duty - LA_DUTY_STEP);
        Serial.print("LA duty: "); Serial.println(la_current_duty);
        delay(100);
    } else if (retractPressed && la_current_duty + LA_DUTY_STEP <= LA_DUTY_MAX) {
        la_write(la_current_duty + LA_DUTY_STEP);
        Serial.print("LA duty: "); Serial.println(la_current_duty);
        delay(100);
    }
...
// Bit ordering: LSB (bit 0) = 1280Ω, bit 1 = 640Ω, bit 2 = 320Ω ... bit 7 = 11Ω
void loadLoadArray(uint8_t loadValue) {
    for (int i = 0; i < 8; i++) {
        loadArray[i] = (loadValue >> i) & 1;
    }
}
...
void state_hill_climb_pitch() {
    ...
    if (la_current_duty + HILL_CLIMB_LA_STEP <= LA_DUTY_MAX) {
