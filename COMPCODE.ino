...
// NOTE: LA values are inverted due to optical isolation on the LA line.
// Raw servo range is 1000[low/retract] to 2000[high/extend].
// After optical isolation inversion: 0[high/extend] to 1000[low/retract].
// Inversion formula: inverted = 2000 - raw
// Physical extend = LOWER value, physical retract = HIGHER value.
const int LA_EXTEND        = 0;     // Hard extend limit (inverted) in microseconds
const int LA_RETRACT       = 1000;  // Hard retract limit (inverted) in microseconds
const int LA_HOME_POSITION = 485;   // Home/default position in microseconds (midpoint)
const int LA_STEP_SIZE     = 10;    // Step size in microseconds - tune for desired speed //#FLAG tune after mechanical testing

// #FLAG - Confirm safe e-stop LA position with EE/mechanical team before competition.
// Remember: lower value = more extended, higher value = more retracted.
// Use formula: E_STOP_POSITION = 2000 - raw_safe_position
const int E_STOP_POSITION  = 500;   // PLACEHOLDER - verify before competition
...
struct LookupEntry {
    ...
    int     la_position;      // target LA position for this wind speed (optical-isolation-corrected)
...
// Pitch (la_position) values are inverted for optical isolation using formula: 2000 - raw_pitch
//   5 m/s: 2000 - 1515 = 485
//   6 m/s: 2000 - 1485 = 515
//   7 m/s: 2000 - 1470 = 530
//   8 m/s: 2000 - 1470 = 530
//   9 m/s: 2000 - 1435 = 565
//  10 m/s: 2000 - 1430 = 570
...
const LookupEntry lookupTable[LOOKUP_TABLE_SIZE] = {
    // wind_speed, la_position, resistance_ohms, load_byte, rpm_spike_delta
    {  5.0f,       485,         1280.0f,         0x80,      200.0f }, //#FLAG tune spike delta
    {  6.0f,       515,         88.0f,           0xF0,      200.0f }, //#FLAG tune spike delta
    {  7.0f,       530,         15.1f,           0xF8,      200.0f }, //#FLAG tune spike delta
    {  8.0f,       530,         15.1f,           0xF8,      200.0f }, //#FLAG tune spike delta
    {  9.0f,       565,         13.29f,          0xD6,      200.0f }, //#FLAG tune spike delta
    { 10.0f,       570,         11.04f,          0xFF,      200.0f }, //#FLAG tune spike delta
    { 11.0f,       570,         11.04f,          0xFF,      200.0f }, // mirrors 10 m/s //#FLAG tune spike delta
    { 12.0f,       570,         11.04f,          0xFF,      200.0f }, // mirrors 10 m/s //#FLAG tune spike delta
    { 13.0f,       570,         11.04f,          0xFF,      200.0f }, // mirrors 10 m/s //#FLAG tune spike delta
};
...
volatile unsigned long prevPulseInterval = 0;  // for spike detection
...
// for per-windspeed spike threshold lookup
...
void ir_interrupt() {
    ...
    // Debounce - ignore pulses less than 10ms apart
    ...
    // Load disconnect detection via RPM spike.
    // A shorter interval = higher RPM. If interval dropped significantly,
    // RPM spiked, which likely means load disconnected (less drag = runaway).
    if (prevPulseInterval > 0 && pulseInterval < prevPulseInterval) {
        // Convert interval ratio to RPM delta and compare against per-windspeed threshold
        ...
        Serial.println("THE LOAD HAS BEEN DISCONNECTED (or at least I think it has been...).");
    }
}
...
// NOTE: With optical isolation inversion, LA_EXTEND (0) < LA_RETRACT (1000).
// A legal write is any value in [0, 1000].
...
void handle_actuator_buttons() {
    ...
    // NOTE: extend decrements position value due to optical isolation inversion
...
void handle_estop_button() {
    ...
    // Trigger on the falling edge (button press), not held state,
    // to avoid repeatedly hammering eStop() every loop iteration.
...
void eStop() {
    ...
    // NOTE: direction logic accounts for optical isolation inversion.
    // Lower us value = more extended, so moving toward E_STOP_POSITION
    // may mean incrementing or decrementing depending on current position.
...
// Updates current_lookup_index to match estimated_wind_speed.
// Called whenever wind speed changes so ir_interrupt() uses the correct spike threshold.
...
void state_estop() {
    ...
    float rpm = get_rpm();
    // Only recover once hardware button released and RPM is safe
    if (digitalRead(ESTOP_READ_PIN) == HIGH && rpm < RPM_MAX_THRESHOLD) {
        load_disconnected = false; // clear flag before re-entering normal operation
...
void state_lookup() {
    ...
    // NOTE: direction logic accounts for optical isolation inversion -
    // a higher la_position value means more retracted, not more extended.
...
void state_hill_climb_pitch() {
    ...
    // NOTE: extending = decrementing position value due to optical isolation inversion
...
// At 11-13 m/s the lookup table mirrors the 10 m/s entry (max load, best pitch).
// This state walks the load DOWN until RPM stabilizes, approximating the same
// output wattage as 10 m/s without voltage/current sensing.
// RPM is used as a proxy: target RPM is derived from the RPM/V constant in tunnel data (~111).
void state_maintain_wattage() {
    ...
    // At higher wind speeds with fixed pitch, RPM will rise above 10 m/s baseline.
    // Increase load to bring RPM back down toward the 10 m/s operating point.
    // #FLAG - replace TARGET_RPM with average of best 10 m/s RPM readings after competition testing.
    // Current value taken from best tunnel data point (V=15.74, pitch=1415 row).
...
void handle_record_button() {
    ...
    if (last_btn_state == HIGH && btn_state == LOW) {
        if (!is_recording) {
            ...
            File f = SD.open(current_filename, FILE_WRITE);
            if (f) {
                f.println("timestamp_ms,rpm,wind_speed_ms,la_position,load_byte");
                f.close();
                is_recording = true;
...
void loop() {
    // Check load disconnect flag first - highest priority after hardware e-stop
